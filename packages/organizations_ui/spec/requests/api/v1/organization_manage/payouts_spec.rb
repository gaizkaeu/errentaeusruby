...
RSpec.describe 'OrganizationManagePayout' do
  ...
  context 'with org_manage user' do
    ...
    it 'renders a successful response' do
      authorized_get api_v1_organization_manage_payouts_url(organization_manage_id: organization.id), as: :json
      expect(response).to be_successful
      expect(body['data']).to be_present
    end

    it 'shows payouts' do
      l = create(:payout, organization:)
      authorized_get api_v1_organization_manage_payouts_url(organization_manage_id: organization.id), as: :json
      expect(response).to be_successful
      expect(body['data']).to be_present
    ...
    it 'can show only authorized payouts' do
      create(:payout, organization:)
      create(:payout, organization: organization2)

      authorized_get api_v1_organization_manage_payouts_url(organization_manage_id: organization2.id), as: :json

      expect(response).not_to be_successful
      expect(response).to have_http_status(:forbidden)
    end

    it 'shows correct payouts' do
      l = create(:payout, organization:)
      create(:payout, organization: organization2)
      authorized_get api_v1_organization_manage_payouts_url(organization_manage_id: organization.id), as: :json
      expect(response).to be_successful
