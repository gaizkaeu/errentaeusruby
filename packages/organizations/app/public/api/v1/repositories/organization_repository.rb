  ...
  def self.map_record(record)
    super(record) do
      if record.logo.attached?
        Api::V1::Organization.new(record.attributes.symbolize_keys!.merge({ logo: Rails.application.routes.url_helpers.rails_blob_url(record.logo) if record.logo.attached? }))
      else
