class Api::V1::LawyerProfileRecord < ApplicationRecord
  ...
  validate :rejected_lawyer_set_off_duty
  validate :rejected_lawyer_cannot_be_updated
  ...
  def rejected_lawyer_set_off_duty
    return unless lawyer_status == 'rejected'

    self.org_status = :off_duty
  end

  def rejected_lawyer_cannot_be_updated
    return unless lawyer_status_was == 'rejected' && lawyer_status == 'rejected'
