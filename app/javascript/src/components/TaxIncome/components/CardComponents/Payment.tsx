        ...
        <div>
            <Card variant="flat">
                <Card.Body>
                    ...
                    <Text>El precio que ha establecido tu abogado es de <Text b color="green">{props.taxIncome.price/100.0} €</Text>.</Text>
                </Card.Body>
            </Card>
            <Spacer />
            {clientSecret ? (
...
export const PaymentCompleted = (props: { taxIncome: TaxIncome }) => {
    ...
    const { currentData, isError, isLoading } = useGetPaymentDataOfTaxIncomeQuery(props.taxIncome.id)
    return !isLoading ? (
        <div>
            {!isError ? (
                <Fragment>
                <Card variant="flat">
                    <Card.Body>
                        ...
                        <CheckAnimated />
                        <Button onPress={() => nav(`/mytaxincome/${props.taxIncome.id}/${props.taxIncome.state}`)}>
                            Continuar
                        </Button>
                    </Card.Body>
                </Card>
